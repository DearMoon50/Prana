...
class PranaApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const PranaDashboard(),
    );
  }
}

class PranaDashboard extends StatefulWidget {
  const PranaDashboard({super.key});

  @override
  State<PranaDashboard> createState() => _PranaDashboardState();
}

class _PranaDashboardState extends State<PranaDashboard> {
  ...
  Future<void> _calculateRisk() async {
    ...
    if (lat == null || lon == null || heatOffset == null) {
      setState(
        () =>
            _statusMessage =
                'Enter valid latitude, longitude, and heat offset.',
      );
      return;
    }
    ...
    try {
      final uri = Uri.parse('${_apiController.text.trim()}/risk/current');
