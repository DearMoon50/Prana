...
def test_mock_returns_scripted_responses_in_order():
    ...
    assert asyncio.run(p.chat([Message(Role.USER, "x")])).content == "first"
    assert asyncio.run(p.chat([Message(Role.USER, "y")])).content == "second"


def test_mock_records_calls():
    p = MockProvider(responses=[ChatResponse(content="ok")])
    asyncio.run(p.chat([Message(Role.USER, "hi")], temperature=0.7))
...
def test_mock_raises_when_configured():
    ...
    with pytest.raises(ProviderError):
        asyncio.run(p.chat([Message(Role.USER, "x")]))
